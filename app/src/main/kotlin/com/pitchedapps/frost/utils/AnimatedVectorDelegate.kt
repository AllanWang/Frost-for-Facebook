 ...
 *
 * Delegate for animated vector drawables with two states (start and end) Drawables are added lazily
 * depending on the animation direction, and are verified upon load Should the bounded view not have
 * an animated drawable upon animating, it is assumed that the user has switched the resource
 * themselves and the delegate will not switch the resource
 */
...
class AnimatedVectorDelegate(
  ...
  /**
   * The delegate will automatically set the start resource when bound If [emitOnBind] is true, it
   * will also trigger the listener
   */
  val emitOnBind: Boolean = true,
  /** The optional listener that will be triggered every time the avd is switched by the delegate */
  override var animatedVectorListener: ((avd: AnimatedVectorDrawable, forwards: Boolean) -> Unit)? =
    null
  ...
  override fun bind(view: ImageView) {
    ...
    view.context.drawable(avdStart) as? AnimatedVectorDrawable
      ?: throw IllegalArgumentException(
        "AnimatedVectorDelegate has a starting drawable that isn't an avd"
      )
    view.context.drawable(avdEnd) as? AnimatedVectorDrawable
      ?: throw IllegalArgumentException(
        "AnimatedVectorDelegate has an ending drawable that isn't an avd"
      )
  ...
  private fun animateImpl(toStart: Boolean) {
    ...
    if (avd == null)
      return L.d { "AVD null resource" } // no longer using animated vector; do not modify
    avd?.stop()

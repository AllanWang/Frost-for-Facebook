...
fun SettingsActivity.getNotificationPrefs(): KPrefAdapterBuilder.() -> Unit = {
  text(R.string.notification_frequency, prefs::notificationFreq, { prefs.notificationFreq = it }) {
    ...
    onClick = {
      materialDialog {
        ...
        listItemsSingleChoice(items = texts, initialSelection = options.indexOf(item.pref)) {
          _,
          index,
          _ ->
  ...
  checkbox(
    ...
    prefs::notificationsGeneral,
    {
      prefs.notificationsGeneral = it
      reloadByTitle(R.string.notification_general_all_accounts)
      if (!prefs.notificationsInstantMessages) reloadByTitle(R.string.notification_frequency)
    }
  ) {
  ...
  checkbox(
    R.string.notification_general_all_accounts,
  ...
  checkbox(
    R.string.notification_messages,
    prefs::notificationsInstantMessages,
    {
      prefs.notificationsInstantMessages = it
      reloadByTitle(R.string.notification_messages_all_accounts)
      if (!prefs.notificationsGeneral) reloadByTitle(R.string.notification_frequency)
    }
  ) {
  ...
  checkbox(
    R.string.notification_messages_all_accounts,
    prefs::notificationsImAllAccounts,
  ...
  if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    checkbox(
      ...
      prefs::notificationSound,
      {
        ...
        reloadByTitle(R.string.notification_ringtone, R.string.message_ringtone)
      }
    )
    ...
    fun KPrefText.KPrefTextContract<String>.ringtone(code: Int) {
      ...
      textGetter = {
        if (it.isBlank()) string(R.string.kau_default)
          ...
          RingtoneManager.getRingtone(this@getNotificationPrefs, frostUri(it))
        ...
        val intent =
          Intent(RingtoneManager.ACTION_RINGTONE_PICKER).apply {
            ...
            if (item.pref.isNotBlank()) {
              putExtra(RingtoneManager.EXTRA_RINGTONE_EXISTING_URI, frostUri(item.pref))
            }
          }
    ...
    checkbox(
      ...
      prefs::notificationVibrate,
      { prefs.notificationVibrate = it }
    )

    checkbox(
      R.string.notification_lights,
      prefs::notificationLights,
      { prefs.notificationLights = it }
    )
  }

  if (BuildConfig.DEBUG) {
    plainText(R.string.reset_notif_epoch) { onClick = { launch { notifDao.deleteAll() } } }
  }

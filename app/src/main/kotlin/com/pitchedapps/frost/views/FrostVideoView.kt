 ...
 *
 * VideoView with scalability Parent must have layout with both height & width as match_parent
 */
class FrostVideoView
@JvmOverloads
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
  ...
  companion object {
    ...
    /**
     * Padding between minimized video and the parent borders Note that this is double the actual
     * padding as we are calculating then dividing by 2
     */
    ...
    set(value) {
      ...
      if (field) {
        ProgressAnimator.ofFloat {
            ...
            withEndAction { if (!isPlaying) showControls() else viewerContract.onControlsHidden() }
          }
          .start()
      } else {
        hideControls()
  ...
  /**
   * Store the boundaries of the minimized video, and return the necessary transitions to get there
   */
  ...
  init {
    ...
    setOnCompletionListener { if (repeat) restart() else viewerContract.onVideoComplete() }
    ...
    setOnVideoSizedChangedListener { intrinsicWidth, intrinsicHeight, pixelWidthHeightRatio ->
      ...
      val shouldRemap =
        !isExpanded &&
          (videoDimensions.x != ratio * intrinsicWidth ||
            videoDimensions.y != ratio * intrinsicHeight)
  ...
  override fun pause() {
    ...
    videoViewImpl.pause()
    keepScreenOn = false
    if (isExpanded) videoControls?.updatePlaybackState(false)
  }
  ...
  private fun hideControls() {
    if (videoControls?.isVisible == true) videoControls?.hide(false)
  }

  private fun toggleControls() {
    if (videoControls?.isVisible == true) hideControls() else showControls()
  }
  ...
  fun destroy() {
    ...
    if (alpha > 0f)
      ProgressAnimator.ofFloat {
        ...
        .start()
    else onFinishedListener()
  }

  private fun onHorizontalSwipe(offset: Float) {
    val alpha = max((1f - abs(offset / SWIPE_TO_CLOSE_OFFSET_THRESHOLD)) * 0.5f + 0.5f, 0f)
    this.alpha = alpha
  }
  ...
  private inner class FrameTouchListener(context: Context) :
    ...
    override fun onSingleTapConfirmed(event: MotionEvent): Boolean {
      if (!viewerContract.onSingleTapConfirmed(event)) toggleControls()
      return true
    }
    ...
    override fun onTouch(view: View, event: MotionEvent): Boolean {
      ...
      when (event.actionMasked) {
        ...
        MotionEvent.ACTION_UP -> {
          if (onSwipe) {
            if (abs(baseSwipeX - event.rawX) > SWIPE_TO_CLOSE_OFFSET_THRESHOLD) destroy()
            else
              animate()
                .translationX(baseTranslateX)
                .setDuration(FAST_ANIMATION_DURATION)
                .withStartAction { animate().alpha(1f) }
          }
        }
      }
      return true

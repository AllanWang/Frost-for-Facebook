...
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
  ...
  companion object {
    ...
    /**
     * Simplified binding to add video to layout, and remove it when finished This is under the
     * assumption that the container allows for overlays, such as a FrameLayout
     */
    fun showVideo(
      url: String,
      repeat: Boolean,
      ...
      videoViewer.binding.video.onFinishedListener = {
        container.removeView(videoViewer)
        contract.onVideoFinished()
      }
      return videoViewer
    }
  }
  ...
  /** Handle back presses returns true if consumed, false otherwise */
  fun onBackPressed(): Boolean {
    with(binding) {
      parent ?: return false
      if (video.isExpanded) video.isExpanded = false else video.destroy()
      return true
    }
  }
...
interface FrostVideoViewerContract : VideoControlsVisibilityListener {
  ...
  /** Process of expansion 1f represents an expanded view, 0f represents a minimized view */
...
interface FrostVideoContainerContract {
  /** Returns extra padding to be added from the right and from the bottom respectively */
